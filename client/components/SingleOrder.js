            ...
            <div>
              <form className="ui form" onSubmit={this.handleOrderUpdate}>
                <div className="field">
                  ...
                  <select className="ui fluid search dropdown" name="status" onChange={evt => this.setState({ [evt.target.name]: evt.target.value })}>
                    <option value="">--</option>
                    {orderStatuses.map((status, idx) => {
                      return (
                        <option key={parseInt(idx.toString(), 10)} value={status}>{status}</option>
                      )
                    })}
                  </select>
                </div>
